//#region Classes
let BLOB_WORKER_SOURCE = null;
function make_worker(url) {
  if (BLOB_WORKER_SOURCE === null) {
    const xhr = new XMLHttpRequest();
    xhr.open("GET", url, false);
    xhr.send();
    if (xhr.status !== 200 && xhr.status !== 0) {
      throw new Error(`worker source fetch failed: ${xhr.status}`);
    }
    BLOB_WORKER_SOURCE = xhr.responseText;
  }
  const blob = new Blob([BLOB_WORKER_SOURCE], { type: "text/javascript" });
...
class RPCWorker {
  constructor(name) {
    ...
    this.worker = make_worker("src/worker.js");
